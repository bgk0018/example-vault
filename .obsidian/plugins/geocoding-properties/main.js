...
var GeocodingSearchModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian2.Setting(inputContainer).setName("Name").addText((text) => {
...
// src/search-property-setting.ts
...
var SearchPropertySetting = class extends import_obsidian3.Setting {
  constructor(plugin, settingTab, containerEl, value, index) {
    super(containerEl);
    this.addText(
      (cb) => cb.setValue(value).setPlaceholder("Property name").onChange(async (newValue) => {
...
// src/settings-tab.ts
var GeocodingPluginSettingTab = class extends import_obsidian4.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian4.Setting(containerEl).setName("Properties").setHeading();
    for (const [key, description] of Object.entries(
      propertyDescriptions
    )) {
      const property = this.plugin.settings.properties[key];
      new import_obsidian4.Setting(containerEl).setName(description.name || key).setDesc(description.detail || "").addText(
    ...
    new import_obsidian4.Setting(containerEl).setName("Override existing properties").setDesc(
    ...
    new import_obsidian4.Setting(containerEl).setName("Map link provider").setDesc("Provider for the map_link property, if enabled").addDropdown(
    ...
    new import_obsidian4.Setting(containerEl).setName("Search property order").setDesc(
    ...
    this.plugin.settings.searchPropertyOrder.forEach(
      (property, index) => new SearchPropertySetting(
        this.plugin,
        this,
        containerEl,
        property,
    ...
    const newSearchInput = new import_obsidian4.Setting(containerEl).addText((cb) => {
    ...
    new import_obsidian4.Setting(containerEl).setName("API").setHeading();
    new import_obsidian4.Setting(containerEl).setName("API provider").addDropdown(
      (dropdown) => dropdown.addOptions({
    ...
    new import_obsidian4.Setting(containerEl).setName("API key").setDisabled(
...
// src/main.ts
var GeocodingPlugin = class extends import_obsidian7.Plugin {
  async onload() {
  ...
  getSearchTerm(file) {
    ...
    const order = ((_a = this.settings.searchPropertyOrder) == null ? void 0 : _a.length) ? this.settings.searchPropertyOrder : defaultSettings.searchPropertyOrder;
    for (const rawKey of order) {
      ...
      const stringValue = typeof value === "string" ? value : Array.isArray(value) ? value.join(", ") : String(value);
      if (stringValue.trim()) {
        return stringValue;
      }
    }
  ...
  async getResults(searchTerm) {
    ...
    if (!(results == null ? void 0 : results.length)) {
      new import_obsidian7.Notice(`No results found for "${searchTerm}"`);
    }
    return results;
  }
